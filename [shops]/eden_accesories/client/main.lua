...
AddEventHandler('eden_accesories:silencieux', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used < silencieux then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Вы использовали ~r~глушитель~s~."))
	...
	used = used + 1
		  		  ...
		  		  ESX.ShowNotification(("У вас нет оружия в руках или ваше оружие не поддерживает глушитель."))
		  		
			end
			else
					  		 ESX.ShowNotification(("Вы должны использовать глушители.")) 
...
AddEventHandler('eden_accesories:flashlight', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used2 < flashlight then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Вы только что экипировали свое оружие фонариком.")) 
		  		 	used2 = used2 + 1
		  	elseif currentWeaponHash == GetHashKey("WEAPON_PISTOL50") then
	...
	used2 = used2 + 1
		  		  ...
		  		  ESX.ShowNotification(("У вас нет оружия в руке или ваше оружие не поддерживает фонарик"))
		  		
			end
		else
				  		  ESX.ShowNotification(("Вы можете использовать все фонарики."))
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used3 < grip then
			...
			if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
		  		 ...
		  		 ESX.ShowNotification(("Вы только что экипировали свое оружие рукояткой.")) 
		  				used3 = used3 + 1
...
AddEventHandler('eden_accesories:yusuf', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used4 < yusuf then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Вы только что экипировали свое оружие скином.")) 
		  		 	used4 = used4 + 1
	...
	used4 = used4 + 1
		  		  ...
		  		  ESX.ShowNotification(("У вас нет оружия в руках или не поддерживает скины."))
		  		
			end
		else
				  		  ESX.ShowNotification(("Вы должны использовать все свои роскошные скины."))
