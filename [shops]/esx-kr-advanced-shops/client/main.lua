...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Скрыть ВСЕ магазины на карте', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Показать ВСЕ магазины на карте', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'У вас есть: ' .. data[1].money .. '$ в вашем магазине',    value = ''})
		table.insert(elements, {label = 'Заказать товар',    value = 'shipments'})
        table.insert(elements, {label = 'Продать предмет', value = 'putitem'})
        table.insert(elements, {label = 'Забрать предмет с продажи',    value = 'takeitem'})
        table.insert(elements, {label = 'Внести деньги на счет магазина',    value = 'putmoney'})
        table.insert(elements, {label = 'Вывести деньги из со счета',    value = 'takemoney'})
        table.insert(elements, {label = 'Изменить название магазина: ' .. Config.ChangeNamePrice..'$',    value = 'changename'})
		table.insert(elements, {label = 'Продать магазин за ' .. math.floor(data[1].ShopValue / Config.SellValue)..'$',   value = 'sell'})
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'НАПИШИТЕ: (YES) без скобок для подтверждения'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Заказать товар', value = 'buy'})
	table.insert(elements, {label = 'Приходы', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = 'Получить заказанный товар', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--В ожидании отгрузки--'})
				once = false
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' за $' .. Config.Items[i].price .. ' a piece ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
...
local name = data.current.ItemName
    ...
    if data.current.value == 'removeitem' then
        ...
        title = 'Сколько бы вы хотели вывезти?'
        }, function(data2, menu2)
    ...
    for i=1, #result.items, 1 do
      ...
      local invitem = result.items[i]
	    ...
	    if invitem.count > 0 then
			table.insert(elements, { label = invitem.label .. ' | ' .. invitem.count .. ' в твоей сумке', count = invitem.count, name = invitem.name})
	    end
	end
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			if count > invcount then
				ESX.ShowNotification('~r~Вы не можете продать больше, чем у вас есть')
				menu2.close()
				menu.close()
			else
				menu2.close()
		...
		ESX.TriggerServerCallback('esx_kr_shop-robbery:getOnlinePolices', function(results)
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
					...
					ESX.ShowNotification("~r~На сервере необходимо минимум полицейских " .. results .. '/' .. Config.RequiredPolices)
				end
			else
				ESX.ShowNotification("~r~Магазин уже ограблен, подождите " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' сек.')
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "Взломать сейф~r~ " .. hp/100 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", '~r~Ваш магазин ~b~(' .. Name ..')~r~ был ограблен!', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~Ваш магазин ~b~(" .. Name .. ')~g~ не удалось ограбить!', Id)
