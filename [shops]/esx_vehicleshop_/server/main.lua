...
RegisterCommand('transfer', function(source, args)
	...
	if(GetPlayerName(tonumber(args[1])))then
            ...
            TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Неверный идентификатор игрока!")
			return
	end
	...
	MySQL.Async.fetchAll(
        ...
        function(result)
            if result[1] ~= nil then
				...
				CarOwner = playerName
				print("Трансфер ", myID, CarOwner)
				if myID == CarOwner then
					print("Transfered")
					
					data = {}
						TriggerClientEvent('chatMessage', other, "^*^1" .. plate .. "^4 Номерной знак автомобиля ^r^4 был передан вам: ^*^2" .. myName)
			 
						MySQL.Sync.execute("UPDATE owned_vehicles SET owner=@owner WHERE plate=@plate", {['@owner'] = targetSteam, ['@plate'] = plate})
						TriggerClientEvent('chatMessage', source, " ^*^1" .. plate .. "\" Вы передали ^r^транспортное средство номерной знак ^r^ игроку ^r^ ^*^2".. targetSteamName)
				else
					print("Did not transfer")
					TriggerClientEvent('chatMessage', source, "^*^1У вас нет транспортного средства.")
				end
			else
				TriggerClientEvent('chatMessage', source, "^1^*ERROR: ^r^0Номер не существует или допущена ошибка.")
            end
