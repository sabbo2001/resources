...
local garages = { 
	...
	[5] = { locked = false, camera = {x = 105.825, y = 6627.562, z = 31.787, heading = 266.692}, driveout = {x = 112.326,y = 6625.148, z = 31.073, heading = 224.641}, drivein = {x = 119.0, y = 6618.33, z = 31.31, heading = 45.5}, outside = {x = 118.493,y = 6618.897, z = 31.13, heading = 224.701}, inside = {x = 109.28, y = 6628.04, z = 31.25, heading = 44.5}},--Beakers Garage Paleto
	[6]= { locked = false, camera = {x = -215.518, y = -1329.135, z = 30.89, heading = 329.092}, driveout = {x = -205.96, y = -1323.32, z = 30.79, heading = 358.5}, drivein = {x = -205.626,y = -1314.99, z = 30.247, heading = 179.395}, outside = {x = -205.594,y = -1304.085, z = 30.614, heading = 358.5}, inside = {x = -212.368,y = -1325.486, z = 30.176, heading = 141.107} }-- Benny's Motorshop
}
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		--Если вы еще не в гараже
		if inside == false then
			local ped = LocalPed()
			--Ну ... да ... нам на самом деле нужна машина, чтобы что-то сделать
			if IsPedSittingInAnyVehicle(ped) then
				local veh = GetVehiclePedIsUsing(ped)
				--Если автомобиль существует, игрок находится на месте водителя, и если это автомобиль или мотоцикл, тогда мы готовы
						...
						if LSC_Config.oldenter then			
							...
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(0,201) then
												...
												drawTxt("Нажмите ~b~ENTER ~w~для входа в ~b~Los Santos Customs ",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										else
											drawTxt("~r~Обслуживаем клиента, пожалуйста подождите!",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										if IsControlJustPressed(0,201) then
											inside = true
									...
									drawTxt("~r~Это транспортное средство запрещено",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
...
local function PointCamAtBone(bone,ox,oy,oz)
	SetIbuttons({
			...
			{GetControlInstructionalButton(1,0, 0),"Свободная камера"}
	 },0)
	SetCamActive(cam, true)
	local veh = myveh.vehicle
...
function LSCMenu:OnMenuChange(last,current)
	...
	elseif IsCamActive(cam) then
		...
		SetIbuttons({
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_back, 0),"Назад"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_select, 0),"Выбрать"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_up, 0),"Вверх"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_down, 0),"Вниз"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_left, 0),"Влево"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_right, 0),"Вправо"}
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("Ваше транспортное средство отремонтировано.")
		else
			LSCMenu:showNotification("Вы сделали покупку.")
		end
		return true
	else
		LSCMenu:showNotification("~r~Вы не можете позволить себе эту покупку.")
		return false
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if LSCMenu:isVisible() then
			...
			local vehicle = GetVehiclePedIsIn( -1 )	
			DisableControlAction(0, 75, true)  -- Disable exit vehicle
			DisableControlAction(27, 75, true) -- Disable exit vehicle
            DisableControlAction( 0, 80, true ) -- Cinematic View
			DisableControlAction(0, 47, true)  -- Disable weapon
			DisableControlAction(0, 68, true)  -- Aim in vehicle
			DisableControlAction(0, 25, true) -- Aim
			DisableControlAction(0, 73, true) -- duck
