...
AddEventHandler('esx_aiTaxi:callTaxi', function(coords)
	if customer then
		ESX.ShowHelpNotification('Такси уже в пути')
	else
		customer = coords
		-- get best spawnpoint
		...
		if customer ~= nil then
			...
			if vehicle == globalTaxi then
				...
				if not DoesBlipExist(waypoint) and not onTour then
					ESX.ShowHelpNotification('Куда едем?')
					Citizen.Wait(2000)
				else
			...
			if customer ~= nil then
				...
				if distanceMeTaxi <= 40 then
					if not parkingDone then
						...
						TriggerEvent('esx:showNotification', 'Ваше такси подъехало')
					end
	...
	while true do
		...
		if customer ~= nil then
			if taxiArrived and not inCar and not onWayBack then
				ESX.ShowHelpNotification('Укажите ~INPUT_PICKUP~ пункт назначения')
...
function atTarget(cancel)
	...
	if cancel then
		...
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		if vehicle ~= globalTaxi then
			TriggerEvent('esx:showNotification', 'Такси было отменено')
			cancelTaxi = true
		else
			TriggerEvent('esx:showNotification', 'Такси не может быть отменено')
			return
		end
	end
	if not cancelTaxi then
		ESX.ShowHelpNotification('Мы прибыли в пункт назначения')
...
function drive(x, y , z, delete, status)
	if status == 'start' then
		...
		ESX.ShowHelpNotification('Такси уже в пути к вам.')
	elseif status == 'end' then
		ESX.ShowHelpNotification('Спасибо за ваше доверие.')
	end
