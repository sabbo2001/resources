...
AddEventHandler('esx_fbijob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'fbi' then
		print(('esx_fbijob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_fbijob:drag', function(target)
	...
	if xPlayer.job.name == 'fbi' then
		...
		print(('esx_fbijob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_fbijob:buyWeaponArmory', function(source, cb, amount)
...
ESX.RegisterServerCallback('esx_fbijob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_fbijob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	-- if price == 0 then
		-- print(('esx_fbijob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		-- cb(false)
	-- else
		if xPlayer.getMoney() >= price then
...
ESX.RegisterServerCallback('esx_fbijob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_fbijob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
