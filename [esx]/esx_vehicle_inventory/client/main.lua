...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0, Keys["F5"]) and (GetGameTimer() - GUI.Time) > 150 then
            ...
            if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
			  ...
			  if locked == 1 or locked == 2 or class == 15 or class == 16 or class == 14 then
	              ...
	              TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
	          	   ...
	          	   ESX.ShowNotification('Багажник ~r~закрыт')
              end
            end
        else
        	ESX.ShowNotification('Нет ~r~транспорта~w~ по близости')
          end
      lastOpen = true
...
--[[
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('Вес : ~g~'.. Kgweight .. ' кг / '..MaxVh..' кг')
              else
                ESX.ShowNotification('Вы достигли предела ~r~ '..MaxVh..' кг')
              end
				    else
