...
function Bag()
    ...
    table.insert(elements, {label = 'Взять предмет', value = 'take'})
    table.insert(elements, {label = 'Бросить сумку', value = 'drop'})
    ...
    while true do
        ...
        for i=1, #Bags, 1 do
            ...
            if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                ...
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~ забрать сумку')
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.35, '~o~[N]~w~ найти сумку')
