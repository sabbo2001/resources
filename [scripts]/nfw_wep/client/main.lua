...
AddEventHandler('nfw_wep:silencieux', function()
    ...
    if WepHash == GetHashKey("WEAPON_PISTOL") then
        ...
        exports['mythic_notify']:DoHudText('inform', 'Вы оснастили оружие глушителем!')
        used = used + 1
    ...
    elseif table.includes(supp4, WepHash) then
        ...
        exports['mythic_notify']:DoHudText('inform', 'Это оружие не совместимо с глушителем')
...
AddEventHandler('nfw_wep:flashlight', function() 
    ...
    if table.includes(flash1, WepHash) then
        ...
        exports['mythic_notify']:DoHudText('inform', 'Вы оснастили оружие фонариком!')
    elseif table.includes(flash2, WepHash) then
        GiveWeaponComponentToPed(GetPlayerPed(-1), WepHash, 0x7BC4CDDC)
        exports['mythic_notify']:DoHudText('inform', 'Вы оснастили оружие фонариком!')
    else
        exports['mythic_notify']:DoHudText('inform', 'Это оружие не совместимо с фонариком')
        TriggerServerEvent('returnItem', item)
    end
end)
...
AddEventHandler('nfw_wep:grip', function()
    ...
    if table.includes(grip1, WepHash) then
        ...
        exports['mythic_notify']:DoHudText('inform', 'Вы оснастили оружие дополнительной рукояткой!')
    else
        exports['mythic_notify']:DoHudText('inform', 'Оружие не поддерживает кастомизацию')
        TriggerServerEvent('returnItem', item)
    end
end)
...
AddEventHandler('nfw_wep:SmallArmor', function()
    ...
    if(armor >= 100) or (armor+30 > 100) then
        exports['mythic_notify']:DoHudText('inform', 'Ваша броня уже увеличена!')
        TriggerServerEvent('returnItem', item)
        return
    end
    ...
    TriggerEvent('mythic_progbar:client:progress', {
        name = 'smallb_armor',
        duration = 5000,
        label = 'Вы одеваете легкий бронижелет...',
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
...
AddEventHandler('nfw_wep:MedArmor', function()
    ...
    TriggerEvent('mythic_progbar:client:progress', {
        name = 'medb_armor',
        duration = 10000,
        label = 'Вы одеваете средний бронижелет...',
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
            disableMovement = true,
...
AddEventHandler('nfw_wep:HeavyArmor', function()
    ...
    TriggerEvent('mythic_progbar:client:progress', {
        name = 'heavyb_armor',
        duration = 15000,
        label = 'Вы одеваете  тяжелый бронижелет...',
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
            disableMovement = true,
        ...
        if not status then
            ...
            exports['mythic_notify']:DoHudText('inform', 'Вы надели легкий бронижелет.')
        end
    end)
end)
...
AddEventHandler('nfw_wep:pAmmo', function()
    ...
    if table.includes(pistol, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', 'Добавлено 50 патронов к пистолету')
        AddAmmoToPed(ped, currentWeaponHash, 50)
    else
        exports['mythic_notify']:DoHudText('inform', 'Это оружие не совместимо с боеприпасами')
        TriggerServerEvent('returnItem', item)
    end
end)
...
AddEventHandler('nfw_wep:mgAmmo', function()
    ...
    if table.includes(mg, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', 'Добавлено 50 патронов к пулемету')
        AddAmmoToPed(ped, currentWeaponHash, 50)
    else
        exports['mythic_notify']:DoHudText('inform', 'Это оружие не совместимо с боеприпасами')
        TriggerServerEvent('returnItem', item)
...
AddEventHandler('nfw_wep:arAmmo', function()
    ...
    if table.includes(ar, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', 'Добавлено 50 патронов для винтовки')
        AddAmmoToPed(ped, currentWeaponHash, 50)
    else
...
AddEventHandler('nfw_wep:sgAmmo', function()
    ...
    if table.includes(sg, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', 'Добавлено 50 патронов к дробовику')
        AddAmmoToPed(ped, currentWeaponHash, 50)
    else
        exports['mythic_notify']:DoHudText('inform', 'This weapon is not compatible with this ammo')
        TriggerServerEvent('returnItem', item)
