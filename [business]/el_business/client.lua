...
function OpenCreateBusinessMenu()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create',
    {
        ...
        elements = {
            ...
            {label = 'Название: ', action = 'name', value = '', key = 2},
            {label = 'Описание: ', action = 'desc', value = '', key = 3},
            {label = 'Адрес: ', action = 'address', value = '', key = 4},
            {label = 'Blip name: ', action = 'blipname', value = '', key = 5},
            {label = 'Цена: ', action = 'price', value = '', key = 6},
            {label = 'Доход: ', action = 'earnings', value = '', key = 7},
            {label = 'Цена акции: ', action = 'stock_price', value = '', key = 8},
            {label = 'Налоговая ставка: ', action = 'taxrate', value = '', key = 9},
            {label = '<span style="color:red;">Налоговая ставка % 0.1=10% - 1.0=100%</span>', action = ''},
            {label = 'Создать бизнес', action = 'create'},
            {label = 'Покинуть бизнес', action = 'discard'}
        }
    }, function(data,menu)
        if data.current.action == 'name' then
            data.current.value = OpenParameterDialog(255)
            data.current.label = 'Название: '..data.current.value
        elseif data.current.action == 'desc' then
            data.current.value = OpenParameterDialog(75)
            data.current.label = 'Описание: '..data.current.value
        elseif data.current.action == 'address' then
            data.current.value = OpenParameterDialog(255)
            data.current.label = 'Адрес: '..data.current.value
        elseif data.current.action == 'price' then
            data.current.value = tonumber(OpenParameterDialog(11))
            data.current.label = 'Стоимость: '..tostring(data.current.value)
        elseif data.current.action == 'earnings' then
            data.current.value = tonumber(OpenParameterDialog(11))
            data.current.label = 'Доход: '..tostring(data.current.value)
        ...
        elseif data.current.action == 'stock_price' then
            data.current.value = tonumber(OpenParameterDialog(11))
            data.current.label = 'Цена акции: '..tostring(data.current.value)
        elseif data.current.action == 'taxrate' then
            data.current.value = tonumber(OpenParameterDialog(11))
            data.current.label = 'Налоговая ставка: '..tostring(data.current.value)
        elseif data.current.action == 'create' then
            ...
            if not error then
                ...
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create_actionpos', {title="Please select the boss action location",align="bottom-right",elements={{label='<span style="color:red;">Your current position will be used</span>',action=''},{label="Set position",action="setpos"},{label="Cancel",action="cancel"}}},function(data1,menu1) if data1.current.action=="setpos" then 
                    ...
                    ESX.ShowNotification("~b~Создание бизнеса...")
                    menu1.close()
                    ESX.TriggerServerCallback("el_business:createBusiness", function(diditwork)
                        if diditwork then ESX.ShowNotification("~g~Бизнес создан!") else ESX.ShowNotification("~r~Произошла ошибка при создании бизнеса") end
                    end,business)
                elseif data1.current.action=="cancel" then menu1.close() end end,function(data1,menu1) menu1.close() end)
                menu.close()
            else
                ESX.ShowNotification("~r~Ошибка, проверьте заполненные данные.")
            end
