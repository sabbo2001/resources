            ...
            for _, v in pairs(Config.RentalLocations) do
                ...
                if PlayerDistance <= 20 then
                    ...
                    if PlayerDistance <= 3 then
                        ESX.ShowHelpNotification("Нажмите ~INPUT_CONTEXT~ чтобы арендовать транспорт") --Hint
                        ...
                        elseif IsControlJustPressed(0, 38) and IsPedInAnyVehicle(GetPlayerPed(-1), false) then  --If they are in a vehicle
                            TriggerClientEvent('esx:showNotification', "~r~Выйдите из машины!")
                        end
                    end
                end
                ...
                if PlayerDistanceVR <= 20 and IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                    ...
                    if PlayerDistanceVR <= 1.5 and IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                        --ESX.ShowHelpNotification("Нажмтие ~INPUT_CONTEXT~ чтобы вернуть транспорт") --Hint
                        TriggerClientEvent('esx:showNotification', "Нажмтие ~INPUT_CONTEXT~ чтобы вернуть транспорт")
                        ...
                        if IsControlJustReleased(0, 38) and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) then
                            ...
                            for _, rc in pairs(Config.RentalCars) do
                                ...
                                for __, c in pairs(Config.CommercialRentals) do
                                    ...
                                    local CR = CRI['model']
                                    --Проверяет, пытается ли игрок вернуть автомобиль напрокат или нет
                                    if GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), false)) == GetHashKey(RC) or GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), false)) == GetHashKey(CR) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) then
                                        ...
                                        TriggerClientEvent('esx:showNotification', "Вы вернули арендованный транспорт!")
                                    end
                                end
                            end
                        end
...
function OpenRentVehicleMenu(x, y, z, heading, type)
    ...
    if type == 'Rental' then    -- If it is a car rental location then insert those vehicles in the UI
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'rent_vehicle',
    {
      ...
      function (data, menu)
        ...
        elseif DoesEntityExist(closevehicle) then   --If there is a vehicle in the way, then an error message will appear
            TriggerClientEvent('esx:showNotification', "Что то блокирует выезд из гаража!")
