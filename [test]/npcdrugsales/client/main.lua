          ...
          if IsPedInAnyVehicle(ped) == false then
            ...
            if pedType ~= 28 and IsPedAPlayer(ped) == false then
              ...
              if distance <= 2 and ped  ~= GetPlayerPed(-1) and ped ~= oldped then
                ...
                if has == true then
                  drawTxt(0.90, 1.40, 1.0,1.0,0.4, "Нажмите ~g~LEFTSHIFT + E ~w~для продажи наркотиков", 255, 255, 255, 255)
                  if IsControlJustPressed(1, 21) and IsControlJustPressed(0, 38) then
                      oldped = ped
...
AddEventHandler('notifyc', function()
    ...
    if crossing ~= nil then
        ...
        ESX.ShowNotification('Позвонили в полицию')
        TriggerServerEvent('esx_addons_gcphone:startCall', 'police', "Продажа накркотиков по адресу " .. streetName .. " и " .. crossing, PlayerCoords, {
