...
function UnJail()
	...
	ESX.ShowNotification("Ваш срок заключения ~g~завершен")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		while jailTime > 0 do
			...
			ESX.ShowNotification("Срок отбывания наказания осталось ~r~" .. jailTime .. " ~w~мин")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "пакет"

					if not v["state"] then
						PackageText = "Уже занят"
					end

					ESX.Game.Utils.DrawText3D(v, "Нажмите [~g~E~w~] чтобы упаковать " .. PackageText, 1.0)
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Вы уже отнесли эту коробку!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "Нажмите [~g~E~w~] чтобы открыть меню", 1.0)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			ESX.ShowNotification("Отменено!")

			Packaging = false
		end

		if PackPercent >= 10 then
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[~g~E~w~] Положить коробку", 1.0)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Отправить в тюрьму", value = "jail_closest_player" },
				{ label = "Освободить из тюрьмы", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Время должно быть в минутах!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Нет игроков поблизости!")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Укажите причину заключения игрока!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Нет заключенных игроков!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Заключенный: " .. playerArray[i].name .. " | Срок: " .. playerArray[i].jailTime .. " минут", value = playerArray[i].identifier })
				end
